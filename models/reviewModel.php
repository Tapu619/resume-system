...
// Get all resumes assigned to THIS specific reviewer
function getAssignedResumes($conn, $reviewer_id) {
    
    // Get the list of resumes assigned to this reviewer
    ...
    // Loop through each resume to find the Seeker's Name
...
// Get details of ONE specific resume for reviewing
function getResumeDetails($conn, $resume_id) {
    
    // Get the resume data from the 'resumes' table
...
// Submit the Review (Score + Comment)
function submitReview($conn, $resume_id, $reviewer_id, $score, $comments) {
    ...
    if (mysqli_query($conn, $sql_insert)) {
        //Update 'resumes' table status to 'reviewed'
